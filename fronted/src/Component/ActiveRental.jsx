...
import { getDetailes } from '../redux/userSlice';
...
function ActiveRental() {
    ...
    const purchaseHistory2 = useSelector((state) => state.purchaseHistoryReducer)
    const purchaseHistory = purchaseHistory2.purchases;

    // console.log("purchaseHistory2  ", purchaseHistory2);
    // console.log("purchaseHistory  ", purchaseHistory);

    // בדיקה אם רכישות נטענו
    const isLoaded = Array.isArray(purchaseHistory?.data);
    const p = isLoaded ? purchaseHistory.data.filter((purchase) => purchase.pointsBalance > 0) : [];
        ...
        <>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'flex-start', mt: 2, mb: 2 }}>
                <Button
                    ...
                    onClick={() => navigate('/purchaseHistory')}>הכל </Button>
            </Box>
            ...
            >
                {
                    // p.length === 0 ? navigate('/Subscriptions', { state: { size: p.length } }) :
